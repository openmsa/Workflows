...
if __name__ == "__main__":
    ...
    try:
        ...
        for i in range(start, stop):
            nic_name = context['net_iface_name'] + '-' + str(i) + '-nic'
            nic = one.nic_create(net_iface_name=nic_name,
                                 public_ip_id=context['pub_ip_id_new'][:-4] + str(i) + '-ip',
                                 subnet_id=context['scale_subnet_id'],
                                 sec_group_id=context['scale_sec_group_id'])
            nicid = nic['id']
            one.get_status_loop(nicid, '2020-05-01')
            status = one.get_resource_status(nicid, '2020-05-01')
    ...
    context['nicid_new'] = nicid

    ret = MSA_API.process_content('ENDED', f'{context["scale_lvl"]} NICs created.', context, True)
    print(ret)