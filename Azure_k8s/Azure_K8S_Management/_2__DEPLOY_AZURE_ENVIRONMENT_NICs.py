...
if __name__ == "__main__":
    ...
    try:
        ...
        for i in range(int(context['vm_qty'])):
            num = i + 1
            nic_name = context['net_iface_name'] + '-' + str(num) + '-nic'
            nic = one.nic_create(net_iface_name=nic_name,
                                 public_ip_id=context['pub_ip_id'][:-4] + str(num) + '-ip',
                                 subnet_id=context['snet_id'],
                                 sec_group_id=context['sg_id'])
            nicid = nic['id']
    ...
    ret = MSA_API.process_content('ENDED', f'NICs created', context, True)
    print(ret)