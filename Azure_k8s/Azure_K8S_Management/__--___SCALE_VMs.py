...
if __name__ == "__main__":
    ...
    try:
        ...
        ret = MSA_API.process_content('WARNING',
                                      f'AZConnection - Connection Error - Check Internet',
                                      context, True)
        print(ret)
        exit()
    except HTTPError:
        ret = MSA_API.process_content('WARNING',
                                      f'AZConnection - HTTP Error - Check API',
                                      context, True)
        print(ret)
        exit()
    ...
    try:
        for i in range(start, stop):
            ...
            vm = one.vm_create(vm_name=vmname,
                               ...
                               username=context['vm_username'],
                               password=context['vm_secret'],
                               nic_id=context['nicid_new'][:-5] + str(i) + '-nic')
            vm_id_list.append(vm['id'])
    
    except ConnectionError:
        ret = MSA_API.process_content('WARNING',
                                      f'AZConnection.vm_create - Connection Error - Check Internet',
                                      context, True)
        print(ret)
        exit()
    except HTTPError:
        ret = MSA_API.process_content('WARNING',
                                      f'AZConnection.vm_create - HTTP Error - Check API',
                                      context, True)
        print(ret)
        exit()
    ...
    Orchestration.update_asynchronous_task_details(*async_update_list,
                                                   f'VMs have just been STARTED, waiting...')
    ...
    try:
        ...
        ret = MSA_API.process_content('WARNING',
                                      f'AZConnection.get_resource_status - Connection Error - Check Internet',
                                      context, True)
        print(ret)
        exit()
    except HTTPError:
        ret = MSA_API.process_content('WARNING',
                                      f'AZConnection.get_resource_status - HTTP Error - Check API',
                                      context, True)
        print(ret)
        exit()

    ret = MSA_API.process_content('ENDED', f'Deployment Completed', context, True)
    print(ret)