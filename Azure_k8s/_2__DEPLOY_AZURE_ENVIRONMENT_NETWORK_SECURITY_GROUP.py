...
if __name__ == "__main__":
    ...
    try:
        ...
        sg = one.sec_group_create(sec_gr_name=context['sec_gr_name'])
        sg_id = sg['id']
        one.get_status_loop(sg_id, '2020-05-01')
        status = one.get_resource_status(sg_id, "2020-05-01")

    except ConnectionError as e:
        ret = MSA_API.process_content('WARNING',
                                      f'Connection Error - Check Internet {e}',
                                      context, True)
        print(ret)
        exit()
    except HTTPError:
    ...
    ret = MSA_API.process_content('ENDED', f'{context["sec_gr_name"]} = {status} ', context, True)
    print(ret)