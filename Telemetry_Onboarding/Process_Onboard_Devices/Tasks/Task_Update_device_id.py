...
'''.format(me_ip,me_ubi_id)
    ...
    try:
        ...
        if int(output_code) < 400:
            ret = MSA_API.process_content('ENDED', f' device_id update in kibana index {context["kibana_index_name"]} successful', context, True)
        else : 
            ret = MSA_API.process_content('FAIL', f' device_id update in kibana index failed with message: {response.text}', context, True)
