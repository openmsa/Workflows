...
if __name__ == "__main__":
    ...
    # (2) Create Manager
    try:
        # user email as username, login first part of email
        r = call_msa.create_user(context['email'],
                                 context['email'].split('@')[0],
                                 context['password'],
                                 context['tenant_prefix']
                                 )
        decoded = r.content.decode('utf-8')
        serialized = json.loads(decoded)
        context["user_id"] = serialized["id"]
    except Exception as e:
        ret = MSA_API.process_content('WARNING',
                                      f'CAN\'T CREATE MANAGER, SEE: {str(e)}',
                                      context, True)
        print(ret)
        exit()
