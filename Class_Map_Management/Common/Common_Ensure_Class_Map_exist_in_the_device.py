...
if response:
    if object_id in response.get(object_name):
        ...
        ret_acl_name = response.get(object_name).get(object_id).get('access').get('0').get('acl')

#if response equals empty dictionary it means class map object is not exist in the device yet.
if not ret_acl_name and is_class_map_name != True:
    ret = MSA_API.process_content(constants.FAILED, 'Class Map with id="' + object_id + '" does not exist in the device.', context, True)
    print(ret)
