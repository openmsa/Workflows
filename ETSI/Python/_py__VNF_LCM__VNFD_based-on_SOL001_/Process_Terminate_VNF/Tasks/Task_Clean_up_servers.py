...
if __name__ == "__main__":
    ...
    for index, vnfc_dict in enumerate(vnf_me_list):
        ...
        #obj=conn.compute.find_server(vnf_resource_id)
        #Stop server by id.
        r = conn.compute.delete_server(vnf_resource_id)
        #context.update(DEBUG_STOP_VDU=r)
        #wait vdu instance status = Shutdown
        #conn.compute.wait_for_delete(obj)
        for j, port_id in enumerate(vnfc_dict['ports']):
            p=conn.network.delete_port(port_id)
        vnf_me_list.remove(vnfc_dict)
        

    MSA_API.task_success('The VNF instance is now  Terminated', context)