...
* 2.FUNCTION : dcconnectChildProcess.php
*/
require_once dirname(__FILE__). '/../NalChildProcess.php';

class dcconnectChildProcess extends NalChildProcess {
    ...
    protected function checkJobCenterNal( $retry, $max ) {
        ...
        if( $status === neccsNal_Config::JOB_STATUS_DONE ) {
            ...
            $dcChk  = isset( $result['data']['wim_check_dcconnect'] ) ? $result['data']['wim_check_dcconnect'] : '';
    ...
    protected function callJobCenterForDb( $method = '' ){
        ...
        if( $method === neccsNal_Config::HTTP_POST ){
            $execCmd = "$jcCmd -p $jobParam ". neccsNal_Config::CREATE_DC_FINAL;

        // In the case of delete
        } else if ( $method === neccsNal_Config::HTTP_DELETE ){
            $execCmd = "$jcCmd -p $jobParam ". neccsNal_Config::DELETE_DC_FINAL;

        // In the case of update
        } else if ( $method === neccsNal_Config::HTTP_PUT ){
            $execCmd = "$jcCmd -p $jobParam ". neccsNal_Config::UPDATE_DC_FINAL;
        }
    ...
    protected function callJobSchedulerForDb() {
        ...
        // In the case of create
        if( $this->_httpMethod === neccsNal_Config::HTTP_POST ){
            $this->jobCenterOperation = neccsNal_Config::CREATE_DC_FINAL;

        // In the case of delete
        } else if ( $this->_httpMethod === neccsNal_Config::HTTP_DELETE ){
            $this->jobCenterOperation = neccsNal_Config::DELETE_DC_FINAL;

        // In the case of update
        } else if ( $this->_httpMethod === neccsNal_Config::HTTP_PUT ){
            $this->jobCenterOperation = neccsNal_Config::UPDATE_DC_FINAL;
        }
