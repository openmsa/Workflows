...
from nec_portal.dashboards.project.node \
...
class DetailView(tables.DataTableView):
    ...
    def get_data(self):
        ...
        tanant_lan = filter(lambda network: network.network_type_detail == '1',
                            detail_node[detail_info['network_name']])
...
class CreateView(forms.ModalFormView):
    form_class = node_forms.CreateNodeForm
    form_id = "create_node_form"
    modal_header = _("Create Node")
    template_name = constants.NODE_CREATE_VIEW_TEMPLATE
    success_url = reverse_lazy(constants.NODE_INDEX_URL)
    page_title = _("Create Node")
    submit_label = _("Create Node")
    submit_url = reverse_lazy(constants.NODE_CREATE_URL)
...
class UpdateView(forms.ModalFormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'description' in NODE_UPDATE_COLUMNS[update_type]:
            context['description'] = \
                NODE_UPDATE_COLUMNS[update_type]['description']
