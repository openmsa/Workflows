...
class TestSelectAPI(unittest.TestCase):
    ...
    def create_fixtures(self):
        ...
        extension_info = {
            'dc_id': 'dc101',
            'ce01_ip_address': '10.0.0.1',
            'ce02_ip_address': '10.0.0.2',
        ...
        cur.execute("INSERT INTO WIM_DC_SEGMENT_MNG(create_id, " +
                    "create_date, update_id, update_date, delete_flg, " +
                    "extension_info) VALUES " +
                    "(%s, %s, %s, %s, %s, %s)", param_vals)

        cur.execute('SELECT last_insert_id() FROM WIM_DC_SEGMENT_MNG')
        global ID
        ID = cur.fetchall()[0][0]
    ...
    def destroy_fixtures(self):
        ...
        cur.execute("DELETE FROM WIM_DC_SEGMENT_MNG WHERE " +
    ...
    def test_delete_api(self):
        ...
        con, cur = self.connect_db()
        cur.execute("SELECT ID FROM WIM_DC_SEGMENT_MNG WHERE ID = %s",
                    [ID])
